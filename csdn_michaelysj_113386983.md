...
set log_dir=%1%
set "exe_dir_7z=C:\Program Files\7-Zip\7z.exe"
set "exe_dir_tshark=C:\Program Files\Wireshark\tshark.exe"
...
rem for /f "usebackq" %%i in (`dir /b %log_dir%\*.tgz`) do (
rem #####################7z
rem bat      %~f0%
rem path     %log_dir%

rem filename %%i
rem mkdir %log_dir%\%%~ni
rem "%exe_dir_7z%" e %log_dir%\%%i -o%log_dir%\%%~ni -aoa
rem "%exe_dir_7z%" e %log_dir%\%%~ni\%%~ni.tar -o%log_dir%\%%~ni -aoa
rem del %log_dir%\%%~ni\%%~ni.tar
rem )

for /f "usebackq delims=" %%a in (`dir /b /s %log_dir%\*.pcap`) do (
rem #####################tshark
rem filename "%%a"
echo "%%a" >> %log_dir%\all.log
"%exe_dir_tshark%" -r "%%a" -R "sip" -2 >> %log_dir%\all.log
)
